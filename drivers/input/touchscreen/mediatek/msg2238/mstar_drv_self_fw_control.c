...
u8 needEnableTpProximity=0;//20151125-add-by-jiangjingjing-defect1171281
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        ...
        {
            nUpdateBinMajor = msg2238_each_update_bin[0xBFF5]<<8 | msg2238_each_update_bin[0xBFF4];
            nUpdateBinMinor = msg2238_each_update_bin[0xBFF7]<<8 | msg2238_each_update_bin[0xBFF6];
        }
	/*
        else if (eSwId == MSG22XX_SW_ID_YYYY) //for other vendor
        {
        ...
        if (nUpdateBinMinor > nMinor)
        {
            if (eSwId == MSG22XX_SW_ID_EACH)
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg2238_each_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
...
{
#if 0  //delete for pixi4 only use msg2238.
    ...
    if (nCrcMainA == nCrcMainB) 
    {
        ...
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
            nUpdateBinMajor = msg2xxx_other_update_bin[31][0x34F]<<8 | msg2xxx_other_update_bin[31][0x34E];
            nUpdateBinMinor = msg2xxx_other_update_bin[31][0x351]<<8 | msg2xxx_other_update_bin[31][0x350];
#else // By one dimensional array
            nUpdateBinMajor = msg2xxx_other_update_bin[0x7F4F]<<8 | msg2xxx_other_update_bin[0x7F4E];
            nUpdateBinMinor = msg2xxx_other_update_bin[0x7F51]<<8 | msg2xxx_other_update_bin[0x7F50];
#endif
        }
...
s32 DrvFwCtrlEnableProximity(void)
{
	...
	if(isTpInSuspend==1)
	{
		MsDrvInterfaceTouchDeviceResume((struct device *)NULL);
		isTpInSuspend = 0 ;
	}
    /*End   ersen.shang 20160113 resume ctp when enable ps for gsensor cts verfier test*/
    ...
    mutex_lock(&g_Mutex);
    needEnableTpProximity=1;//20151125-add-by-jiangjingjing-defect1171281
    mdelay(I2C_WRITE_COMMAND_DELAY_FOR_FIRMWARE);
    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[0], 4);
    if (rc > 0)
    {
