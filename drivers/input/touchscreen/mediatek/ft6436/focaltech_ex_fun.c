...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	...
	{
		fw_len = sizeof(CTPM_FW_GREEN);
		CTPM_FW = CTPM_FW_GREEN;		
	}
	printk("mtk_tpd[FTS] fw_len id is %d\n",fw_len );
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		...
		pbt_buf = CTPM_FW;
			printk("mtk_tpd[FTS] sizeof(CTPM_FW) is %d\n",fw_len );
...
u8 fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] == 0x79) {
			DBG("[FTS] Step 3: GET CTPM ID OK,ID1 = 0x%x\n",
				reg_val[0]);
			break;
		} else {
			dev_err(&client->dev, "[FTS] Step 3: GET CTPM ID FAIL,ID1 = 0x%x\n",
				reg_val[0]);
		}
	}
	...
	buf[0] = 0x3;
	buf[1] = 0x0;
	/*Begin-liuxin-modify according FAE's suggestion-20151105*/
	buf[2] = 0x7;
	buf[3] = 0xb0;
	...
	dev_dbg(&client->dev, "[FTS] old setting: uc_i2c_addr = 0x%x,\
			...
			buf[0], buf[2], buf[4]);
	DBG("[FTS] old setting: uc_i2c_addr = 0x%x,\
			uc_io_voltage = %d, uc_panel_factory_id = 0x%x\n",
			buf[0], buf[2], buf[4]);
	retVal=buf[4];
	...
	msleep(200);
	return /*0*/retVal;
	/*End-liuxin-modify according FAE's suggestion-20151105*/
}
