...
//extern struct input_dev *tpd->dev;
extern struct tpd_device  *tpd;//20160310 liujunting add for hardwareinfo
...
extern int   MSG_tpd_proximity_detect;
...
#elif  1/////defined(CONFIG_TOUCHSCREEN_MTK_HQ_GESTURE)
...
u8 needEnableTpProximity = 0; //add by jiayu.ding for defect1748502 on 20160310     
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
...
#ifdef MSG_GESTURE_SYSFS_MODE
short msg_pointnum = 0;
unsigned char msg_original_coordinate[200] = {0};
#endif
...
static s32 _DrvFwCtrlParsePacket(u8 *pPacket, u16 nLength, TouchInfo_t *pInfo)
{
    ...
    if (g_GestureWakeupFlag == 1)
    {
        ...
        DBG("pPacket[0]=%x \n pPacket[1]=%x pPacket[2]=%x pPacket[3]=%x pPacket[4]=%x pPacket[5]=%x \n", \
	     ...
	     msg_pointnum = 0;
        if (g_ChipType == CHIP_TYPE_MSG22XX && pPacket[0] == 0xA7 /*&& pPacket[1] == 0x00 figer num&& pPacket[2] == 0x06 && pPacket[3] == 0x50*/ )
        {
            //nWakeupMode = pPacket[4];
            nWakeupMode = pPacket[122];
            bIsCorrectFormat = 1;
	        msg_pointnum = pPacket[1];	
        } 
 #else
...
//                    input_report_key(tpd->dev, RESERVER1, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER1, 0);
...
//                    input_report_key(tpd->dev, RESERVER2, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER2, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x6E:
...
//                    input_report_key(tpd->dev, RESERVER3, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER3, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
...
//                    input_report_key(tpd->dev, RESERVER4, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER4, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x70:
...
//                    input_report_key(tpd->dev, RESERVER5, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER5, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x71:
...
//                    input_report_key(tpd->dev, RESERVER6, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER6, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x72:
...
//                    input_report_key(tpd->dev, RESERVER7, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER7, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x73:
...
//                    input_report_key(tpd->dev, RESERVER8, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER8, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x74:
...
//                    input_report_key(tpd->dev, RESERVER9, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER9, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x75:
...
//                    input_report_key(tpd->dev, RESERVER10, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER10, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x76:
...
//                    input_report_key(tpd->dev, RESERVER11, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER11, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x77:
...
//                    input_report_key(tpd->dev, RESERVER12, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER12, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x78:
...
//                    input_report_key(tpd->dev, RESERVER13, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER13, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x79:
...
//                    input_report_key(tpd->dev, RESERVER14, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER14, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x7A:
...
//                    input_report_key(tpd->dev, RESERVER15, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER15, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x7B:
...
//                    input_report_key(tpd->dev, RESERVER16, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER16, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x7C:
...
//                    input_report_key(tpd->dev, RESERVER17, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER17, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x7D:
...
//                    input_report_key(tpd->dev, RESERVER18, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER18, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x7E:
...
//                    input_report_key(tpd->dev, RESERVER19, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER19, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x7F:
...
//                    input_report_key(tpd->dev, RESERVER20, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER20, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x80:
...
//                    input_report_key(tpd->dev, RESERVER21, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER21, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x81:
...
//                    input_report_key(tpd->dev, RESERVER22, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER22, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x82:
...
//                    input_report_key(tpd->dev, RESERVER23, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER23, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x83:
...
//                    input_report_key(tpd->dev, RESERVER24, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER24, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x84:
...
//                    input_report_key(tpd->dev, RESERVER25, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER25, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x85:
...
//                    input_report_key(tpd->dev, RESERVER26, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER26, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x86:
...
//                    input_report_key(tpd->dev, RESERVER27, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER27, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x87:
...
//                    input_report_key(tpd->dev, RESERVER28, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER28, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x88:
...
//                    input_report_key(tpd->dev, RESERVER29, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER29, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x89:
...
//                    input_report_key(tpd->dev, RESERVER30, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER30, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x8A:
...
//                    input_report_key(tpd->dev, RESERVER31, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER31, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x8B:
...
//                    input_report_key(tpd->dev, RESERVER32, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER32, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x8C:
...
//                    input_report_key(tpd->dev, RESERVER33, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER33, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x8D:
...
//                    input_report_key(tpd->dev, RESERVER34, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER34, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x8E:
...
//                    input_report_key(tpd->dev, RESERVER35, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER35, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x8F:
...
//                    input_report_key(tpd->dev, RESERVER36, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER36, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x90:
...
//                    input_report_key(tpd->dev, RESERVER37, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER37, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x91:
...
//                    input_report_key(tpd->dev, RESERVER38, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER38, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x92:
...
//                    input_report_key(tpd->dev, RESERVER39, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER39, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x93:
...
//                    input_report_key(tpd->dev, RESERVER40, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER40, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x94:
...
//                    input_report_key(tpd->dev, RESERVER41, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER41, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x95:
...
//                    input_report_key(tpd->dev, RESERVER42, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER42, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x96:
...
//                    input_report_key(tpd->dev, RESERVER43, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER43, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x97:
...
//                    input_report_key(tpd->dev, RESERVER44, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER44, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x98:
...
//                    input_report_key(tpd->dev, RESERVER45, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER45, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x99:
...
//                    input_report_key(tpd->dev, RESERVER46, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER46, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x9A:
...
//                    input_report_key(tpd->dev, RESERVER47, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER47, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x9B:
...
//                    input_report_key(tpd->dev, RESERVER48, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER48, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x9C:
...
//                    input_report_key(tpd->dev, RESERVER49, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER49, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x9D:
...
//                    input_report_key(tpd->dev, RESERVER50, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER50, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
                case 0x9E:
...
//                    input_report_key(tpd->dev, RESERVER51, 1);
                    input_report_key(tpd->dev, KEY_POWER, 1);
                    input_sync(tpd->dev);
//                    input_report_key(tpd->dev, RESERVER51, 0);
                    input_report_key(tpd->dev, KEY_POWER, 0);
                    input_sync(tpd->dev);
                    break;
#endif //CONFIG_SUPPORT_64_TYPES_GESTURE_WAKEUP_MODE
...
//                    input_report_key(tpd->dev, RESERVER51, 0);
 ...
 #ifdef CONFIG_TOUCHSCREEN_MTK_HQ_GESTURE
	//		printk("tangzugang  terry gesture point msg_original_coordinate= ");
            	for(i = 0;i < 28;i++)
		{
		        msg_original_coordinate[i] = pPacket[i + 2];
    			printk("%d ",msg_original_coordinate[i]);
		}	
 #endif
        }
        else
    ...
    if ((pPacket[nCheckSumIndex] == nCheckSum) && (pPacket[0] == 0x52))   // check the checksum of packet
    {
        ...
        */
		...
		{		
         		if((pPacket[5] == 0x80) )    /////// closing
         		{ 
                 		MSG_tpd_proximity_detect = 0;
         		}
         		else if((pPacket[5] == 0x40) ) ////// leaving
         		{       
                 		MSG_tpd_proximity_detect = 1;
         		}
				
			DBG("msg_ps MSG_tpd_proximity_detect =%d\n", MSG_tpd_proximity_detect);
			
			sensor_data.values[0] = MSG_tpd_proximity_detect;
			sensor_data.value_divide = 1;
			sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
			if ((err = hwmsen_get_interrupt_data(ID_PROXIMITY, &sensor_data)))
			{
				DBG(" msg ps  call hwmsen_get_interrupt_data failed= %d\n", err);	
			}
...
/*begin add by jiayu.ding for TP JD compibility ID 12 and 15 at 20160530*/	
		if (eSwId == 15)
			eSwId = MSG22XX_SW_ID_YYYY;
/*end add by jiayu.ding for TP JD compibility ID 12 and 15 at 20160530*/
        ...
        //begin 20160310 liujunting add for hardwareinfo
	#if defined (CONFIG_HW_INFO)
	tpd->tp_firmware_version = pVersion;
	tpd->tp_vendor_id = eSwId;
	#endif
	//end 20160310 liujunting add for hardwareinfo
...
//    DBG("*** %s() ***\n", __func__);  // add for debug
 ...
 //   DBG("*** %s() *** mutex_lock(&g_Mutex)\n", __func__);  // add for debug
    ...
    if (nReportPacketLength > 8)
    {
        ...
        DmaReset();
#endif //CONFIG_ENABLE_DMA_IIC
	DBG("I2C read packet data length, nReportPacketLength = %d\n", nReportPacketLength);
        rc = IicReadData(SLAVE_I2C_ID_DWI2C, &pPacket[0], nReportPacketLength);
    }
    else
    {
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        if ((tInfo.nFingerNum) == 0)   //touch end
        {
            if (nLastKeyCode != 0)
            {
                ...
                input_report_key(tpd->dev, nLastKeyCode, 0);
        ...
        else //touch on screen
        {
            if (tInfo.nTouchKeyCode != 0)
            {
                ...
                if (nLastKeyCode != nTouchKeyCode)
                {
                    ...
                    input_report_key(tpd->dev, nTouchKeyCode, 1);
...
s32 DrvFwCtrlEnableProximity(void)
{
    ...
    needEnableTpProximity=1;//add by jiayu.ding for defect1748502 on 20160310 
    mdelay(I2C_WRITE_COMMAND_DELAY_FOR_FIRMWARE);
    rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[0], 4);
    if (rc > 0)
    {
...
s32 DrvFwCtrlDisableProximity(void)
{
    ...
    mutex_lock(&g_Mutex);
    needEnableTpProximity=0;//add by jiayu.ding for defect1748502 on 20160310 
