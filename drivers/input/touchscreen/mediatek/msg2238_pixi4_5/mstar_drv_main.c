...
#ifdef MSG_GESTURE_PROC_MODE
extern struct tpd_device *tpd;
#endif

#define TpTpye_HLT    (16) 
#define TpTpye_JD      (12)

u8 TpType;
extern u16 _DrvMpTestItoTestGetTpType(void);
...
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
#ifdef MSG_GESTURE_PROC_MODE
static const struct file_operations _gProcGestureFunInfo = { 
    .read = msg_gesture_read_proc,
    .write = msg_gesture_write_proc,
};

static const struct file_operations _gProcGesturePointInfo = { 
    .read = msg_gesture_point_read,
    .write = msg_gesture_point_write,
};
#endif
...
//add by xunanbin 20151222
ssize_t DrvMainCtpInfoShow(struct kobject *pKObj, struct kobj_attribute *pAttr, char *pBuf)
{
      char tp_info[20]={"no this devices"};

	if (TpType == TpTpye_HLT)
	{
           sprintf(tp_info,"MSG2238_HLT");
	}
	else if (TpType == TpTpye_JD)
	{
	    sprintf(tp_info,"MSG2238_JD");
	}
	printk("jiayu TP_ID TpType = %d\n", TpType);	
	return snprintf(pBuf, sizeof(tp_info), "%s\n", tp_info);
}

static struct kobj_attribute packet_attr = __ATTR(packet, 0664, DrvMainKObjectPacketShow, DrvMainKObjectPacketStore);
static struct kobj_attribute ctp_attr = __ATTR(ctp, 0664, DrvMainCtpInfoShow, NULL);

/* Create a group of attributes so that we can create and destroy them all at once. */
static struct attribute *attrs[] = {
    &packet_attr.attr,
    &ctp_attr.attr,             //add by xunanbin 20151222
...
#define MSG_PROC_READ_GESTRUEY		10
static unsigned char msg_proc_operate_mode = 0;


ssize_t msg_gesture_read_proc(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    DBG("*** %s() ***\n", __func__);

return sprintf(pBuffer, "%d\n", tpd->gesture_enable);

}

ssize_t msg_gesture_write_proc(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)  
{
 char v_gesture_enable;


copy_from_user(&v_gesture_enable, pBuffer, nCount);
if (v_gesture_enable != '0' && v_gesture_enable != '1')
{
	printk("set gesture_enable failed, input should be 1 or 0.\n");
	return -EFAULT;
}

tpd->gesture_enable = v_gesture_enable - '0';
printk("[fanxueliang] get ps state addr %d\n", tpd->gesture_enable);

if(v_gesture_enable - '0' == 1)
	g_GestureWakeupMode[0] = 0x1FFF;
else if(v_gesture_enable - '0' == 0)
...
ssize_t msg_gesture_point_read(struct file *pFile, char __user *pBuffer, size_t nCount, loff_t *pPos)
{
    DBG("*** %s() ***\n", __func__);
	int i, j;
	unsigned char buf[GESTURE_WAKEUP_PACKET_LENGTH];
	int num_read_chars = 0;
	int readlen = 0;
	
	switch (msg_proc_operate_mode) {

	case MSG_PROC_READ_GESTRUEY:

		buf[0] = 0; ////no effect
		for(i = 0;i < 28;i++)
		{
			buf[1 + i] = msg_original_coordinate[i];
		}
		num_read_chars = 29;
	...
	memcpy(pBuffer, buf, num_read_chars);

	return num_read_chars;

}

ssize_t msg_gesture_point_write(struct file *pFile, const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	unsigned char writebuf[GESTURE_WAKEUP_PACKET_LENGTH];
	int buflen = nCount;
	int writelen = 0;
	int ret = 0;
	
	if (copy_from_user(&writebuf, pBuffer, buflen)) {
		printk("read gesture point error\n");
		return -EFAULT;
	}
	msg_proc_operate_mode = writebuf[0];
...
extern void msg_gesture_sysfs(void);
#endif
...
#ifdef MSG_GESTURE_PROC_MODE
//////proc sysfs for firmware
//	DrvIcFwCreateProcNode();
#endif
#ifdef MSG_GESTURE_SYSFS_MODE
msg_gesture_sysfs();
#endif
...
//        tsps_assist_register_callback("msg2xxx", &DrvPlatformLyrTpPsEnable, &DrvPlatformLyrGetTpPsData);
    ...
    mdelay(200); 
/*-begin-2016-05-25-add by jiayu.ding for read ID error-*/
//    TpType = _DrvMpTestItoTestGetTpType();
	do{
		TpType = _DrvMpTestItoTestGetTpType();
		if (TpType == 15)
			TpType = 12;
		Rd_ID_num++;
		printk("jiayu TP ID read TpType = %d Rd_ID_num = %d\n",TpType,Rd_ID_num);
		if (Rd_ID_num >= 6)
			break;
		else if (Rd_ID_num < 5)
			mdelay(100);
			...
			DrvPlatformLyrTouchDeviceResetHw();
	}while((TpType != TpTpye_HLT)&&(TpType != TpTpye_JD));	
  /*-end-2016-05-25-add by jiayu.ding for read ID error-*/
    return nRetVal;
}
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
    ...
    if (NULL == proc_create(MSG_GESTURE_PROC_FILE, PROCFS_AUTHORITY, NULL, &_gProcGestureFunInfo))
    {
        DBG("Failed to create procfs file node(%s)!\n", MSG_GESTURE_PROC_FILE);
    }   
    else 
    {
        DBG("Create procfs file node(%s) OK!\n", MSG_GESTURE_PROC_FILE);
    ...
    if (NULL == proc_create(MSG_GESTURE_POINT_PROC_FILE, 0777, NULL, &_gProcGesturePointInfo))
    {
        DBG("Failed to create procfs file node(%s)!\n", MSG_GESTURE_POINT_PROC_FILE);
    }   
    else 
    {
        DBG("Create procfs file node(%s) OK!\n", MSG_GESTURE_POINT_PROC_FILE);
