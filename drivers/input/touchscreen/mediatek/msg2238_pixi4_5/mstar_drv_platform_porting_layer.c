...
unsigned int msg_touch_irq = 0;
unsigned int msg_tpd_rst_gpio_number = 0;
unsigned int msg_tpd_int_gpio_number = 1;
...
*///<80 900 20 160 240 900 20 160 400 900 20 160 0 0 0 0>;
const int g_TpVirtualKeyDimLocal[MAX_KEY_NUM][4] = {{80,900,20,160},
                                                    {240,900,20,160},
                                                    {400,900,20,160},
                                                    {0,0,0,0}};
#endif 
...
//DBG("***tpd   444 ***\n"); 
    ...
    if (_gInterruptFlag == 1) 
    {    
        disable_irq_nosync(msg_touch_irq);
//DBG("***tpd   5555 ***\n"); 
        _gInterruptFlag = 0;
...
#if 1//ndef CONFIG_USE_IRQ_INTERRUPT_FOR_MTK_PLATFORM
...
//    gpio_set_value(MS_TS_MSG_IC_GPIO_RST, 1); 
	...
	tpd_gpio_output(msg_tpd_rst_gpio_number, 0);
	//tpd_gpio_output(msg_tpd_int_gpio_number, 0);
	msleep(20);
	...
	tpd_gpio_output(msg_tpd_rst_gpio_number, 0);
	msleep(100);
	tpd_gpio_output(msg_tpd_rst_gpio_number, 1);
	
	//tpd_gpio_output(msg_tpd_int_gpio_number, 1);
	//msleep(20);
	//tpd_gpio_as_int(msg_tpd_int_gpio_number);
	msleep(100);
#endif
}
...
void DrvPlatformLyrFingerTouchPressed(s32 nX, s32 nY, s32 nPressure, s32 nId)
{
    ...
    input_mt_slot(tpd->dev, nId);
    input_mt_report_slot_state(tpd->dev, MT_TOOL_FINGER, true);
    input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, 1);
    input_report_abs(tpd->dev, ABS_MT_WIDTH_MAJOR, 1);
    input_report_abs(tpd->dev, ABS_MT_POSITION_X, nX);
    input_report_abs(tpd->dev, ABS_MT_POSITION_Y, nY);
#ifdef CONFIG_ENABLE_FORCE_TOUCH
    input_report_abs(tpd->dev, ABS_MT_PRESSURE, nPressure);
...
//    DBG("nId=%d, nX=%d, nY=%d\n", nId, nX, nY); // TODO : add for debug
    ...
    input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, nId);
...
#if 0///def CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM
#ifdef CONFIG_TP_HAVE_KEY    
...
s32 DrvPlatformLyrInputDeviceInitialize(struct i2c_client *pClient)
{
    ...
    tpd->dev->name = pClient->name;
    tpd->dev->phys = "I2C";
    tpd->dev->dev.parent = &pClient->dev;
    tpd->dev->id.bustype = BUS_I2C;
    ...
    { 
        ...
        {
            input_set_capability(tpd->dev, EV_KEY, g_TpVirtualKey[i]);
        }
    }
...
#ifdef CONFIG_TP_HAVE_KEY
    // Method 2.
    set_bit(TOUCH_KEY_MENU, tpd->dev->keybit); //Menu
    set_bit(TOUCH_KEY_HOME, tpd->dev->keybit); //Home
    set_bit(TOUCH_KEY_BACK, tpd->dev->keybit); //Back
    set_bit(TOUCH_KEY_SEARCH, tpd->dev->keybit); //Search
#endif
*/
...
#if defined(CONFIG_ENABLE_TOUCH_DRIVER_FOR_MUTUAL_IC)
    input_set_abs_params(tpd->dev, ABS_MT_TRACKING_ID, 0, (MAX_TOUCH_NUM-1), 0, 0);
#endif //CONFIG_ENABLE_TOUCH_DRIVER_FOR_MUTUAL_IC
    input_set_abs_params(tpd->dev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(tpd->dev, ABS_MT_WIDTH_MAJOR, 0, 15, 0, 0);
    input_set_abs_params(tpd->dev, ABS_MT_POSITION_X, TOUCH_SCREEN_X_MIN, TOUCH_SCREEN_X_MAX, 0, 0);
    input_set_abs_params(tpd->dev, ABS_MT_POSITION_Y, TOUCH_SCREEN_Y_MIN, TOUCH_SCREEN_Y_MAX, 0, 0);
#ifdef CONFIG_ENABLE_FORCE_TOUCH
    input_set_abs_params(tpd->dev, ABS_MT_PRESSURE, 0, 255, 0, 0);
#endif //CONFIG_ENABLE_FORCE_TOUCH

#ifdef CONFIG_ENABLE_TYPE_B_PROTOCOL
    input_mt_init_slots(tpd->dev, MAX_TOUCH_NUM, INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED);
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL

    /* register the input device to input sub-system */
    nRetVal = input_register_device(tpd->dev);
    if (nRetVal < 0)
    {
...
s32 DrvPlatformLyrTouchDeviceRequestGPIO(struct i2c_client *pClient)
{
	...
	TPD_DMESG("mtk_tpd: tpd_probe msg2xxxmsg_tpd_rst_gpio_number = %d\n",msg_tpd_rst_gpio_number);
...
static int tpd_irq_registration(void)
{
	...
	TPD_DMESG("mstar Device Tree Tpd_irq_registration!");
	...
	if (node) {
		...
		msg_touch_irq = irq_of_parse_and_map(node, 0);

			ret = request_irq(msg_touch_irq, (irq_handler_t) _DrvPlatformLyrFingerTouchInterruptHandler, IRQF_TRIGGER_RISING,
	...
	TPD_DMESG("[%s]irq:%d, debounce:%d-%d:", __func__, msg_touch_irq, ints[0], ints[1]);
	return ret;
}
...
//                      "msg2xxx", NULL); 
	...
	thread = kthread_run(_DrvPlatformLyrFingerTouchHandler, 0, TPD_DEVICE);
	if (IS_ERR(thread)) {
	...
	TPD_DMESG("Tpd int number msg_tpd_int_gpio_number %d\n", msg_tpd_int_gpio_number);
...
//////gpio free 
	gpio_free(msg_tpd_rst_gpio_number);
	gpio_free(msg_tpd_int_gpio_number);
    return 0;
}
