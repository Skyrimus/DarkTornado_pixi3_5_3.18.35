//V1.0.0 Kavie 20160301 MTK6580 SP2508MIPI for TCL pixi4-4 3G
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP2508_MIPI_RAW_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

	.checksum_value = 0x879cb68d,
...
u8 SP2508MIPI_Initialize_from_T_Flash()
{
	...
	fp = filp_open("/system/lib/sp2508_sd", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
	...
	for (j=0; j<i; j++)
	{
		...
		{
			write_cmos_sensor(SP2508MIPI_Init_Reg[j].init_reg, SP2508MIPI_Init_Reg[j].init_val);
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor(0x25,0xf0); //reg dac 2.7v, enable bl_en,vbl 1.4v
    ...
    write_cmos_sensor(0x8a,0x44); //pixel bias 1uA
    write_cmos_sensor(0x8b,0x44); 
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) 
	{
		...
		do 
		{
			*sensor_id = return_sensor_id();
			printk("sp2508_get_imgsensor_id: 0x%x\n", *sensor_id);
...
#ifdef DEBUG_SENSOR_SP2508
	if(fromsd == 1)
		SP2508MIPI_Initialize_from_T_Flash();//
	else
#endif
		sensor_init();
...
static kal_uint32 get_info(MSDK_SCENARIO_ID_ENUM scenario_id,MSDK_SENSOR_INFO_STRUCT *sensor_info,MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	sensor_info->SensorInterruptDelayLines = 4; /* not use */   // 1
