...
typedef struct otp_gc5005{
	...
	kal_uint16 rg_gain;
	kal_uint16 bg_gain;
	kal_uint16 g_rg_gain;
	kal_uint16 g_bg_gain;
	kal_uint16 ob;
	kal_uint16 wb_flag;
...
{
	kal_uint8 flag0,flag_info,flag_awb;
	kal_uint8 i,j,cnt=0;
...
//DD
	...
	gc5005_select_page_otp(otp_page1);
	flag_info = gc5005_read_otp(0x00);
	
	if((flag_info & 0xc0) == 0x40) {
		info_start_add = 0x01;// base address of info group 1
		gc5005_otp_info.info_flag=0x1;
		LOG_INF("GC5005 INFO Group1 is valid");
	} else if((flag_info & 0x30) == 0x10) {
		info_start_add = 0x08;// base address of info group 2
		gc5005_otp_info.info_flag=0x1;
		LOG_INF("GC5005 INFO Group2 is valid");
	} else {
		gc5005_otp_info.info_flag=0x0;
		LOG_INF("GC5005 no OTP");
	}
...
//INFO
	if(info_start_add!=0)
	{
		...
		gc5005_otp_info.vcm_driver_id = gc5005_read_otp(info_start_add + 3);
		gc5005_otp_info.vcm_id = gc5005_read_otp(info_start_add + 2);
	...
	{
		gc5005_otp_info.module_id = 0;
		gc5005_otp_info.lens_id = 0; 
		gc5005_otp_info.vcm_driver_id = 0;
		gc5005_otp_info.vcm_id = 0;
		gc5005_otp_info.year = 0;
		gc5005_otp_info.month = 0;
		gc5005_otp_info.day = 0;
	}
	
	/*print otp information*/
	LOG_INF("GC5005_OTP_INFO:flag=0x%x\n",flag_info);
...
//AWB
	flag_awb = gc5005_read_otp(0x0F);
	if((flag_awb & 0xc0) == 0x40) {
		wb_start_add = 0x10;// base address of info group 1
		gc5005_otp_info.wb_flag=0x1;
		LOG_INF("GC5005 AWB Group1 is valid");
	} else if((flag_awb & 0x30) == 0x10) {
		wb_start_add = 0x1B;// base address of info group 2
		gc5005_otp_info.wb_flag=0x1;
		LOG_INF("GC5005 AWB Group2 is valid");
	} else {
		gc5005_otp_info.wb_flag=0x0;
		LOG_INF("GC5005 no AWB OTP");
	}
	
	if(wb_start_add!=0)
	{
		gc5005_otp_info.ob=gc5005_read_otp(wb_start_add);
		gc5005_otp_info.rg_gain=(gc5005_read_otp(wb_start_add+1)<<8)|(gc5005_read_otp(wb_start_add+2)&0xff);
		gc5005_otp_info.bg_gain=(gc5005_read_otp(wb_start_add+3)<<8)|(gc5005_read_otp(wb_start_add+4)&0xff);
		gc5005_otp_info.g_rg_gain=(gc5005_read_otp(wb_start_add+6)<<8)|(gc5005_read_otp(wb_start_add+7)&0xff);
		gc5005_otp_info.g_bg_gain=(gc5005_read_otp(wb_start_add+8)<<8)|(gc5005_read_otp(wb_start_add+9)&0xff);
	}
	else
	{
		gc5005_otp_info.ob=0;
		gc5005_otp_info.rg_gain=0;
		gc5005_otp_info.bg_gain=0;
		gc5005_otp_info.g_rg_gain=0;
		gc5005_otp_info.g_bg_gain=0;
	}
	
	LOG_INF("GC5005_OTP_WB:wb_flag=0x%x\n",flag_awb);
	LOG_INF("GC5005_OTP_WB:OB=%d\n",gc5005_otp_info.ob);
	LOG_INF("GC5005_OTP_WB:r/g=0x%x\n",gc5005_otp_info.rg_gain);
	LOG_INF("GC5005_OTP_WB:b/g=0x%x\n",gc5005_otp_info.bg_gain);
	LOG_INF("GC5005_OTP_WB:g_r/g=0x%x\n",gc5005_otp_info.g_rg_gain);
	LOG_INF("GC5005_OTP_WB:g_b/g=0x%x\n",gc5005_otp_info.g_bg_gain);
	
	if((gc5005_otp_info.wb_flag==0x1)&&(gc5005_otp_info.wb_flag==0x1)&&(gc5005_otp_info.module_id==0x44)){
		GC5005_OTP_flag=1;
	}
	else{
		GC5005_OTP_flag=0;
...
static void gc5005_gcore_update_awb(void)
{
	...
	if((0==gc5005_otp_info.g_rg_gain)||(0==gc5005_otp_info.g_bg_gain)||(0==gc5005_otp_info.rg_gain)||(0==gc5005_otp_info.bg_gain))
	{
		gc5005_otp_info.wb_flag = 0;
		LOG_INF("GC5005_OTP_UPDATE_AWB:Empty or Invaild!! \n");
	}
	
	if(0x01==(gc5005_otp_info.wb_flag&0x01))
	{	
		r_gain_current = 256 * gc5005_otp_info.g_rg_gain/gc5005_otp_info.rg_gain;
		b_gain_current = 256 * gc5005_otp_info.g_bg_gain/gc5005_otp_info.bg_gain;
		g_gain_current = 256;
...
*
*************************************************************************/
/*Note£ºersen.shang remove otp on minisw 20160913*/
