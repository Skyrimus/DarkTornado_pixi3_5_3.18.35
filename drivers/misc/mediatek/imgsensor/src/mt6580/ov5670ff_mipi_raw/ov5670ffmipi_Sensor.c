...
#include "ov5670ffmipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov5670ff_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670FFMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int ov5670ff_read_otp(void)
{
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+0x000B;
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
				break;
			}
