...
static imgsensor_info_struct imgsensor_info = { 
	...
	/*Begin ersen.shang modify color format for color revert issue*/
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,
	/*End   ersen.shang modify color format for color revert issue*/
	.mclk = 24,
...
static imgsensor_struct imgsensor = {
	/*Begin ersen.shang for HV Mirror 20151120*/
	.mirror = IMAGE_HV_MIRROR,				//mirrorflip information 
	/*End   ersen.shang for HV Mirror 20151120*/
...
/*Begin ersen.shang 20160223 add [0x323b,0x02] for red color reduce*/
...
/*End   ersen.shang 20160223 add [0x323b,0x02] for red color reduce*/
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x320b,0x0E);//20150713
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	LOG_INF("wenzq----Read sensor id 5e8 begin, \n");
	...
	while(imgsensor_info.i2c_addr_table[i] != 0xff) 
	{
		...
		do 
		{
			*sensor_id = return_sensor_id();
			LOG_INF("wenzq----Read sensor id 5e8 begin,sensor_id=%x \n",*sensor_id );
			if (*sensor_id == imgsensor_info.sensor_id) 
			{				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("wenzq----Read sensor id fail, write_id: 0x%x,sensorid=0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
