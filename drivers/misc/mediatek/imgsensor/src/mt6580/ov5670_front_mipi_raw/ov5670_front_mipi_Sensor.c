...
#include "ov5670_front_mipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov5670_front_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670_FRONT_MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x4041, 0x03); //0x00
...
static int ov5670_front_read_otp(void)
{
	...
	return module_id;   //wenzhiquan vcm_id;  because module_id for vendor,vcm_id for af,ff
  }
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id =( ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C))+0x000f);
			 module_id=ov5670_front_read_otp();
		 if(module_id!=0xFF)
		 {
			 if(module_id==0x0c)*sensor_id-=1;              //0x0c: sunrise  0x06 qtech  567e is sunrise,567f is qtech  wenzhiquan
